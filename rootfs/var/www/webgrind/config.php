<?php
/**
 * This file is default webgrind config.php except the xdebug profiler path is changed.
 * Also use 95 FunctionPercentage as default since we usually want to see more.
 */
...
class Webgrind_Config extends Webgrind_MasterConfig {
    ...
    static $defaultTimezone = 'Europe/Helsinki';
    ...
    static function xdebugOutputDir() {
        if ( !empty( getenv( 'XDEBUG_OUTPUT_DIR' ) ) )
            return getenv( 'XDEBUG_OUTPUT_DIR' ) . '/';
    ...
    static function storageDir() {
        ...
        if ( !empty( getenv( 'WEBGRIND_STORAGE_DIR' ) ) )
            return getenv( 'WEBGRIND_STORAGE_DIR' ) . '/';
